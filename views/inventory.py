...
def render():
    ...
    with tab_view:
        ...
        with c3: styled_metric("Locations", len(set(i.get("location", "") for i in inventory)), color="#0e7490")
        ...
        with fc3:
            loc_filter = st.selectbox("Location", ["All"] + sorted(set(i.get("location", "") for i in inventory)), key="inv_loc")
        ...
        if loc_filter != "All":
            filtered = [i for i in filtered if i.get("location") == loc_filter]

        if filtered:
            df = pd.DataFrame(filtered)
            cols = ["item_id", "item_name", "category", "sub_category", "specification", "quantity", "unit", "location", "price"]
        ...
        item_opts = {f"{i['item_name']} | {i.get('specification','')} ({i['item_id']})": i for i in inventory}
        ...
        del_opts = {f"{i['item_name']} | {i.get('specification','')} | Qty: {i.get('quantity',0)} ({i['item_id']})": i for i in inventory}
            ...
            if st.form_submit_button("✅ Add to Inventory", use_container_width=True):
                if sel_mi and qty > 0:
                    ...
                    receive_to_inventory(
                        mi["item_name"], mi.get("category", ""), mi.get("sub_category", ""),
                        mi.get("specification", ""), qty, mi.get("unit", "Nos"),
                        location, mi.get("price", 0))
