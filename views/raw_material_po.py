"""Purchase Orders — clean titles, PDF download, file attachments."""
...
from utils.db import (
    ...
    update_raw_material_po_status, place_po_via_sqs, add_inventory_item,
...
def render():
    ...
    st.markdown("*Create, track, receive, download PDF, and attach files*")
    st.markdown("---")
    ...
    with tab2:
        ...
        if not vendors: st.warning("Register vendors first."); return
        ...
        if vendor_master:
            with st.expander(f"📂 Quick Add from Master Catalog ({len(vendor_master)} items)"):
                for mi in vendor_master[:30]:
                    ...
                    with qc1: st.markdown(f"**{mi['item_name']}** — {mi.get('specification', '')}")
                    with qc2:
                        ...
                        if st.button("Add", key=f"va_{mi['item_id']}"):
                            if qty > 0:
                                st.session_state.po_items.append({"description": mi["item_name"],
                                    "specification": mi.get("specification", ""), "quantity": qty,
                                    "unit": mi.get("unit", "Kg"), "unit_price": price})
                                st.rerun()
    ...
    with tab1:
        ...
        if not all_pos: empty_state("📦", "No purchase orders yet"); return
            ...
            with st.expander(label):
                ...
                if pdf_key:
                    ...
                    if st.button("📄 Generate PDF", key=f"genpdf_{po['po_id']}"):
                        ...
                        if pk: update_po_pdf_key(po["po_id"], pk); st.success("PDF generated!"); st.rerun()

                # Attachments
                st.markdown("**📎 Attachments**")
                att_keys = list_attachments(po["po_id"])
                for ak in att_keys:
                    fname = ak.split("/")[-1]
                    ab = get_attachment(ak)
                    if ab:
                        st.download_button(f"⬇️ {fname}", ab, fname, key=f"att_{ak}")
                uploaded = st.file_uploader("Upload attachment", key=f"up_{po['po_id']}")
                if uploaded:
                    if st.button("📤 Upload", key=f"upbtn_{po['po_id']}"):
                ...
                if po_items:
                    ...
                    for item in po_items:
                        ...
                        with rc2: st.caption(f"Ord: {item.get('quantity', 0)} {item.get('unit', '')}")
                        with rc3: st.caption(f"Rcvd: {item.get('quantity_received', 0)}")
                        with rc4:
                            ...
                            if st.button("Save", key=f"sv_{po['po_id']}_{item['item_id']}"):
                                ...
                                if ir and qr > 0:
                                    add_inventory_item("", item.get("description", ""), po.get("vendor_name", ""),
                                        ...
                                        qr, item.get("unit", "Kg"), "Main Store", item.get("unit_price", 0))
