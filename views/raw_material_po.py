"""Purchase Orders — lazy-loaded details, month folders, cached queries."""
...
def _get_month_key(po):
    ...
    return created[:7] if created and len(created) >= 7 else "Unknown"
...
def _render_po_list(month_pos):
    """Render a list of POs as selectable items — details load only when selected."""
    if not month_pos:
        return

    # Build selection list from PO metadata (no DB calls needed — data already in memory)
    po_labels = {}
    for po in month_pos:
        status = po.get("status", "Draft")
        icon = _status_icon(status)
        po_labels[f"{icon} [{status}] {po['po_id']} — {po.get('vendor_name', '')} | {format_currency(po.get('total_amount', 0))}"] = po

    for label, po in po_labels.items():
        with st.expander(label):
            _render_po_details(po)


def _render_po_details(po):
    """Render full PO details — DB calls happen HERE only when expander is opened."""
    status = po.get("status", "Draft")
    ...
    with pc3:
        ...
        gst = po.get("gst_percentage", 18)
        st.markdown(f"**GST:** {gst}% | **Status:** {status}")

    # PDF — only check S3 key stored on PO record (no extra DB call)
    ...
    # Line items — this is the heaviest call, only fires when expander is open
    po_items = get_raw_material_po_items(po["po_id"])
    if po_items:
        if is_complete:
            ...
            for item in po_items:
                ...
                with rc2:
                    ...
                    st.caption(f"Received: {already_received} | Remaining: {remaining}")
                with rc3:
                    if is_item_done:
                ...
                if not is_item_done and remaining > 0:
                    with st.form(key=f"recv_form_{po['po_id']}_{item['item_id']}"):
                        ...
                        with fc1:
                            recv_now = st.number_input("Receiving now", min_value=0.0, max_value=remaining,
                                value=0.0, step=1.0, key=f"rn_{po['po_id']}_{item['item_id']}")
                        with fc2:
                            mark_complete = st.checkbox("All received — close this item",
                                value=False, key=f"mc_{po['po_id']}_{item['item_id']}")
                        with fc3:
                        ...
                        if submitted and (recv_now > 0 or mark_complete):
                            ...
                            update_po_item_receipt(po["po_id"], item["item_id"], new_total, mark_complete)
                            if recv_now > 0:
                                receive_to_inventory(
                                    item.get("description", ""), item.get("category", "Received"),
                                    item.get("sub_category", "PO Receipt"), item.get("specification", ""),
                                    recv_now, item.get("unit", "Kg"), "Main Store", item.get("unit_price", 0))
                            if mark_complete:
                                st.success(f"Item closed. Total: {new_total}/{ordered}")
                            else:
    ...
    if status == "Draft":
        ...
        with dc2:
            if st.button("🗑️ Delete Draft", key=f"del_{po['po_id']}", use_container_width=True):
                ...
                st.success(f"Draft {po['po_id']} deleted.")
                st.rerun()


def render():
    st.markdown("# 📦 Purchase Orders")
    st.markdown("*Current month shown. Older months in folders.*")
    st.markdown("---")
    ...
    with tab2:
        ...
        gst_pct = st.number_input("GST %", min_value=0.0, max_value=28.0, value=18.0, step=0.5,
                                    help="Default 18%. CGST and SGST split equally.")
            ...
            with st.expander(f"📂 Quick Add from Master Catalog ({len(vendor_master)} items)"):
                for mi in vendor_master[:30]:
                    ...
                    with qc2:
                        ...
                        st.caption(f"INR {price}/{mi.get('unit', '')}")
                    with qc3:
                        ...
                        if st.button("Add", key=f"va_{mi['item_id']}"):
                            if qty > 0:
                                st.session_state.po_items.append({"description": mi["item_name"],
                                    ...
                                    "category": mi.get("category", ""), "sub_category": mi.get("sub_category", ""),
        ...
        with st.form("manual_item", clear_on_submit=True):
            ...
            with mc1: desc = st.text_input("Description *")
            with mc2: spec = st.text_input("Specification")
            with mc3: qty = st.number_input("Qty *", min_value=0, step=1)
            with mc4: unit = st.selectbox("Unit", ["Kg", "Nos", "Meters", "Sets", "Lots", "Pcs"])
            ...
            if st.form_submit_button("➕ Add"):
                if desc and qty > 0:
                    st.session_state.po_items.append({"description": desc, "specification": spec,
                        "category": "", "sub_category": "", "quantity": qty, "unit": unit, "unit_price": price})
                    st.rerun()
        ...
        if st.session_state.po_items:
            ...
            for idx, item in enumerate(st.session_state.po_items):
                ...
                with ic4:
                    if st.button("🗑️", key=f"rem_{idx}"): st.session_state.po_items.pop(idx); st.rerun()
            ...
            with cs:
                if st.button("💾 Save Draft", use_container_width=True):
                    ...
                    for f in (new_attachments or []): upload_attachment(po["po_id"], f.name, f.read(), f.type)
                    st.success(f"PO **{po['po_id']}** saved"); st.session_state.po_items = []; st.rerun()
            with cp:
                if st.button("📤 Place Order", use_container_width=True, type="primary"):
                    ...
                    pk = generate_po_pdf(po, st.session_state.po_items, "Material")
                    if pk: update_po_pdf_key(po["po_id"], pk)
                    for f in (new_attachments or []): upload_attachment(po["po_id"], f.name, f.read(), f.type)
                    st.success(f"PO **{po['po_id']}** placed!"); st.session_state.po_items = []; st.rerun()

    # ─── All POs — month folders ──────────────────────────────
    with tab1:
        ...
        for po in all_pos:
            by_month[_get_month_key(po)].append(po)

        sorted_months = sorted(by_month.keys(), reverse=True)
        current_count = len(by_month.get(current_month, []))
        st.markdown(f"**{len(all_pos)} total** | **{current_count} this month** | **{len(sorted_months)} months**")
        st.markdown("---")
        ...
        for month_key in sorted_months:
            ...
            if month_key == current_month:
                ...
                _render_po_list(month_pos)
                st.markdown("---")
            else:
                # Previous months — fully lazy. Nothing renders until clicked.
